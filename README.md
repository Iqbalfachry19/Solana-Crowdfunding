...
- **`refund()`**: Allows a donor to claim a full refund of their contribution if the campaign failed to reach its `goal` by the `deadline`. Funds are transferred from the `vault` PDA back to the donor's wallet, and the `donation` account is closed with rent returned to the donor.
